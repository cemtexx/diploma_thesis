...
class ScalesCounter():
    def __init__(self, TEMPORARY_IMAGE, CONFIDENCE_THRESHOLD, ENLARGE_BODY, CUT_SIZE, MINIMUM_SCALES, MAXIMUM_SCALES, LOCAL_FILTER, TM, IMAGE_OUTPUT, MINIMUM_DISTANCE) -> None:
        self.error_info = 'OK'
        self.TEMPORARY_IMAGE = TEMPORARY_IMAGE
        self.CONFIDENCE_THRESHOLD = CONFIDENCE_THRESHOLD
        self.ENLARGE_BODY = ENLARGE_BODY
        self.CUT_SIZE = CUT_SIZE
        self.MINIMUM_SCALES = MINIMUM_SCALES
    ...
    def yolo_recognizing(self, image_path, net):
        if image_path[-4:] == '.jpg' or image_path[-4:] == '.JPG' or image_path[-4:] == '.png' or image_path[-4:] == '.PNG':
            ...
            psd.composite().save(self.TEMPORARY_IMAGE)
            raw_image = imread(self.TEMPORARY_IMAGE)[:, :, :3]
            os.remove(self.TEMPORARY_IMAGE)

        elif image_path[-4:] == '.CR2':
            raw = rawpy.imread(image_path)
            rgb = raw.postprocess()
            imageio.imsave(self.TEMPORARY_IMAGE, rgb)
        ...
        if len(found_body) == 0:
            ...
            if found_body[3] >= found_body[4]:  # horizontally
                ...
                cut_image = self._cut_square(
                    raw_image, found_body[1], found_body[2], found_body[5]*self.ENLARGE_BODY, angle)
            else:  # vertically
                angle = 90
                cut_image = self._cut_square(
                    raw_image, found_body[1], found_body[2], found_body[5]*self.ENLARGE_BODY, angle)
    ...
    def _cut_square(self, image, centerX, centerY, lenght, inverse):
        ...
        resized = cv2.resize(
            cut, (self.CUT_SIZE, self.CUT_SIZE), interpolation=cv2.INTER_AREA)
        return resized
    ...
    def u_net(self, cut_image, models):
        ...
        if results[0] > self.MAXIMUM_SCALES or results[0] < self.MINIMUM_SCALES or results[1] > self.MAXIMUM_SCALES or results[1] < self.MINIMUM_SCALES:
            ...
            if results[0] > self.MAXIMUM_SCALES or results[0] < self.MINIMUM_SCALES or results[1] > self.MAXIMUM_SCALES or results[1] < self.MINIMUM_SCALES:
                ...
                if results[0] > self.MAXIMUM_SCALES or results[0] < self.MINIMUM_SCALES or results[1] > self.MAXIMUM_SCALES or results[1] < self.MINIMUM_SCALES:
                    if self.error_info != 'program found just a few scales, image is probably in bad quality':
    ...
    def _counterpoint(self, preds_model, raw_image):
        ...
        while True:
            ...
            try:
                ...
                self.error_info = 'program found just a few scales, image is probably in bad quality'
                
                if self.IMAGE_OUTPUT:
                    final_image = raw_image.copy()
                    return [0, 0, final_image]
                else:
                    return [0, 0]
